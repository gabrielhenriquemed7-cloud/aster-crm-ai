...
alter table public.medical_records
  add column if not exists status public.medical_record_status,
...
set
  status = case
    when appointment.status = 'completed'
      then 'finalized'::public.medical_record_status
    else 'draft'::public.medical_record_status
  end,
  finalized_at = case
    when appointment.status = 'completed'
      then coalesce(record.finalized_at, appointment.updated_at, record.updated_at)
    else record.finalized_at
  end,
  finalized_by = case
    when appointment.status = 'completed'
      then coalesce(record.finalized_by, appointment.professional_id)
    else record.finalized_by
  end,
...
where record.status is null
  and appointment.id = record.appointment_id;
...
begin
  if not exists (
    ...
    where conrelid = 'public.appointments'::regclass
      and conname = 'appointments_completed_by_fkey'
  ) then
    alter table public.appointments
      add constraint appointments_completed_by_fkey
  ...
  if not exists (
    ...
    where conrelid = 'public.medical_records'::regclass
      and conname = 'medical_records_finalized_by_fkey'
  ) then
    alter table public.medical_records
      add constraint medical_records_finalized_by_fkey
...
alter table public.appointments
  validate constraint appointments_completed_by_fkey;
alter table public.medical_records
  validate constraint medical_records_finalized_by_fkey;
...
create index if not exists medical_record_addenda_record_idx
  on public.medical_record_addenda(medical_record_id, created_at);
...
begin
  if not exists (
    ...
    where tgrelid = 'public.medical_records'::regclass
      and tgname = 'medical_records_protect_finalized'
      and not tgisinternal
  ) then
    create trigger medical_records_protect_finalized
    before update on public.medical_records
    for each row
    execute function public.protect_finalized_medical_record();
  end if;
end
$$;

create or replace function public.create_medical_record_addendum(
  target_record_id uuid,
...
begin
  ...
  if source.id is null
    or source.status not in ('finalized', 'amended')
  then
    raise exception using
      errcode = '22023',
      message = 'Apenas prontuários finalizados aceitam adendos.';
  end if;
  ...
  if not exists (
    ...
    where profile.id = current_user_id
      and profile.active_clinic_id = source.clinic_id
  ) then
    raise exception using
      errcode = '42501',
      message = 'Somente administradores da clínica podem registrar adendos.';
  end if;

  if nullif(trim(addendum_content), '') is null
    or nullif(trim(addendum_reason), '') is null
  then
    raise exception using
      errcode = '23514',
      message = 'Informe o conteúdo e o motivo do adendo.';
  end if;

  insert into public.medical_record_addenda (
    medical_record_id,
    clinic_id,
    content,
    reason,
  ...
  values (
    source.id,
    source.clinic_id,
    trim(addendum_content),
    trim(addendum_reason),
    current_user_id
  )
...
begin
  if not exists (
    ...
    where schemaname = 'public'
      and tablename = 'medical_record_addenda'
      and policyname = 'Clinic members view medical record addenda'
  ) then
    create policy "Clinic members view medical record addenda"
      on public.medical_record_addenda
      for select
      to authenticated
  ...
  if not exists (
    ...
    where schemaname = 'public'
      and tablename = 'medical_record_addenda'
      and policyname = 'Clinic admins create medical record addenda'
  ) then
    create policy "Clinic admins create medical record addenda"
      on public.medical_record_addenda
      for insert
      to authenticated
      with check (
        ...
        and exists (
          ...
          where clinic_id = medical_record_addenda.clinic_id
...
revoke all on function public.create_medical_record_addendum(
  uuid,
  text,
  text
) from public;

grant execute on function public.start_clinical_encounter(uuid)
  to authenticated;
grant execute on function public.finalize_clinical_encounter(uuid)
  to authenticated;
grant execute on function public.create_medical_record_addendum(
  uuid,
  text,
  text
) to authenticated;
