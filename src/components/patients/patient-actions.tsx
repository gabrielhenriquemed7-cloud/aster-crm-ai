...
export function PatientActions({ id, name }: { id: string; name: string }) {
  ...
  return <Dialog open={open} onOpenChange={setOpen}><DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" size="icon-sm" aria-label={`Ações para ${name}`}><MoreHorizontal /></Button></DropdownMenuTrigger><DropdownMenuContent align="end"><DropdownMenuItem asChild><Link href={`/patients/${id}/edit`}><Pencil /> Editar</Link></DropdownMenuItem><DialogTrigger asChild><DropdownMenuItem variant="destructive"><Trash2 /> Excluir</DropdownMenuItem></DialogTrigger></DropdownMenuContent></DropdownMenu><DialogContent><DialogHeader><DialogTitle>Excluir paciente?</DialogTitle><DialogDescription>Esta ação removerá permanentemente o cadastro de {name}.</DialogDescription></DialogHeader><DialogFooter><DialogClose asChild><Button variant="outline">Cancelar</Button></DialogClose><Button variant="destructive" onClick={remove} disabled={loading}>{loading ? "Excluindo..." : "Excluir paciente"}</Button></DialogFooter></DialogContent></Dialog>;
}
