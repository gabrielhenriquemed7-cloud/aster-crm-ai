...
export function TeamManager({
  ...
  async function confirmRemoval() {
    ...
    setRemoval(null);
    await perform(selected.id, () =>
      selected.record_type === "invite"
    ...
    <>
      <div className="grid gap-6 xl:grid-cols-[360px_minmax(0,1fr)]">
        <Card className="h-fit shadow-none">
          <CardHeader>
            <CardTitle>Convidar para {clinic?.name}</CardTitle>
          </CardHeader>
          <CardContent>
                ...
                <TableBody>
                  {records.map((record) => {
                    ...
                    const actionableInvite =
                      pending &&
                      ["pending", "sent", "failed", "expired"].includes(
                        record.status,
                      );
                      ...
                      <TableRow key={`${record.record_type}-${record.id}`}>
                        <TableCell>
                          <div className="min-w-44">
                            <p className="font-medium">
                              {record.full_name || record.email || "Usuário"}
                            </p>
                            <p className="text-xs text-muted-foreground">
                              {record.email || "E-mail indisponível"}
                            </p>
                          </div>
                        </TableCell>
                        <TableCell>
                        ...
                        <TableCell>
                          ...
                          {pending && record.expires_at && (
                            <p className="text-xs text-muted-foreground">
                              Validade {formatDate(record.expires_at)}
                            </p>
                          )}
                          {pending && record.invited_by && (
                            <p className="text-xs text-muted-foreground">
                              Por {record.invited_by}
                            </p>
                          )}
                        </TableCell>
                        <TableCell>
