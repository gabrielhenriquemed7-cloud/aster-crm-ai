...
function CopyPage({ document, copy }: { document: ClinicalDocument; copy?: string }) {
  ...
  return <article className="mx-auto min-h-[277mm] max-w-[210mm] bg-white p-[18mm] text-black print:min-h-screen print:max-w-none print:p-[15mm]">
    <header className="flex items-start justify-between border-b-2 border-black pb-4">
      {settings.show_logo !== false && clinic?.logo_url ? <div role="img" aria-label={`Logomarca de ${clinic.name}`} className="h-16 w-36 bg-contain bg-left bg-no-repeat" style={{ backgroundImage: `url(${clinic.logo_url})` }} /> : <div><h1 className="text-xl font-bold">{clinic?.name}</h1><p className="text-xs">{clinic?.legal_name}</p></div>}
      <div className="max-w-64 text-right text-xs"><p>{settings.header_text || clinic?.legal_name}</p><p>{settings.show_cnpj !== false && clinic?.cnpj && `CNPJ ${clinic.cnpj}`}</p><p>{settings.show_address !== false && clinicAddress(document)}</p><p>{settings.show_phone !== false && (clinic?.phone || clinic?.whatsapp)} {settings.show_email !== false && clinic?.email}</p></div>
    </header>
    ...
    <footer className="mt-20 text-center"><div className="mx-auto w-72 border-t border-black pt-2 text-sm"><strong>{professional?.professional_name || professional?.full_name}</strong><p>{professional?.profession || "Médico"}</p>{settings.show_council !== false && <p>{professional?.council} {professional?.council_number}{professional?.council_state ? `/${professional.council_state}` : ""}</p>}{settings.show_specialty !== false && <p>{professional?.specialty}{settings.show_rqe !== false && professional?.rqe ? ` · RQE ${professional.rqe}` : ""}</p>}<p className="mt-8 text-xs">{settings.signature_text || (settings.physical_signature_space !== false ? "Assinatura física e carimbo" : "")}</p></div></footer>
