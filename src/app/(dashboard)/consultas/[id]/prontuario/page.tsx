...
export default async function MedicalRecordPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (data.error || !data.appointment) {
    return <div className="rounded-xl border border-destructive/30 bg-destructive/5 p-8 text-center">
      <AlertCircle className="mx-auto size-8 text-destructive" />
      <h1 className="mt-3 font-semibold">Não foi possível carregar o prontuário</h1>
      <p className="mt-1 text-sm text-muted-foreground">{data.error}</p>
    </div>;
  }
  return <MedicalRecordForm appointment={data.appointment} record={data.record} canEdit={data.canEdit} />;
}
